...
**サブエージェント拒否時のフォールバック:** サブエージェントがブラウザ操作（フォーム送信等）を拒否して処理が進まない場合、そのバッチをメインコンテキストで再実行する。メール送信（`send_and_log.py` 経由）は通常サブエージェントで問題なく動作するため、拒否が起きるのはブラウザ操作のみ。メインでの再実行時はフォーム対象のみ処理し、メール送信済みの営業先は重複しないようDBステータスで判定する。
