...
**完全自動実行:** このスキルはユーザーが意図的に起動したものであり、メール送信・フォーム送信・SNS DM送信のいずれについても、**個別のユーザー確認を一切求めずに実行すること。** 送信前の確認ダイアログ、「送信してよいですか？」等の質問は不要。全件を自動で処理し、結果をまとめてレポートする。
...
- **送信者情報**: 送信元メールアドレス、署名
- **メールテンプレート**: テンプレートが定義されている場合はベースとして使用する
...
**本文の個別化:** 各営業先の `overview`（事業概要）と `match_reason`（マッチ理由）を必ず参照し、冒頭だけでなく本文全体を相手に合わせて書くこと。相手企業の具体的な数値・実績・取り組み・業種に言及し、テンプレートの単純な差し替えにしない。「貴社のウェブサイトを拝見し」等の汎用的な書き出しだけでは不十分。
