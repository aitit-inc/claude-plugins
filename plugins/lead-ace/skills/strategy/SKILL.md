...
2. **未記載・不完全なセクション**: 何が不足しているか具体的に
3. **BUSINESS.md の概要**: 存在するか、主な内容

ユーザーに方針を確認する:
- **「不足分を埋める」**（デフォルト推奨）: 未記載・不完全なセクションの情報のみ収集 → ステップ4で該当ステップのみ実行
- **「特定セクションを更新」**: ユーザーが指定したセクションの情報を再収集
- **「全体を見直す」**: ステップ4の全ステップを実行（既存値をデフォルトとして提示）
...
- **更新モード（不足分を埋める）**: ステップ3で不足と判定されたセクションに対応するステップのみ実行する。充足済みのステップはスキップ
- **更新モード（特定セクション更新）**: ユーザーが指定したセクションに対応するステップのみ実行する。既存値を提示し変更点を確認
- **更新モード（全体見直し）**: 全ステップを実行するが、各ステップで既存値をデフォルトとして提示し「変更ありますか？」と聞く。変更がなければそのまま維持
...
- **更新モード**: 既存の `$0/SALES_STRATEGY.md` を読み込み、変更・追加があったセクションのみ更新する。変更のないセクションはそのまま維持する。ユーザーが明示的に削除を指示しない限り、既存の記載を消さない
